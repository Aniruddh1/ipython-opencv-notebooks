...
class Image_Engine:
    """ Image Engine
    """
    def __init__(self, file_spec, is_folder, color=False):
...
def main():
    ...
    parser.add_argument('-f', dest='is_folder', type=bool, default=False, help='in_file is a folder/glob spec')    
    ...
    #### '../projects/contact_front/MR_NewCam/transitionSet/2015-08-04_DFI28-*.png'

    start_num = 1
    #bg_img = eng.images[start_num-1].copy()
    
    if args.in_file:
        eng = Image_Engine(args.in_file, is_folder=args.is_folder)

    ret, bg_img = eng.next()
    if not ret:
        print "Error opening video file: ", args.in_file
        exit(1)
    ...
    while ret:
        ...
        ellipse, debug = findEllipse(img, center, diag_len, verbose)
        
        if ellipse[0][0] == -99:
            #overlay = fg_img.copy()
            
            print("%s Unable to find contact area, Image index: (%d)%s" % (util.RED, img_idx, util.RESET))
            if do_plot: plot_lst.append( (img, 'findEllipse %d' % (img_idx) ))
            ...
            output = np.concatenate((fg_img, debug), axis=1)
            ...
            #if ellipse[2] == 0:
            if True:
                plot_center = ((center[0] + ellipse[0][1] - height/2), (center[1] + ellipse[0][0] - width/2))
    ...
    #         height = ellipse[1][1] + pad if (ellipse[1][1] + pad) <= rows else rows
            ...
            stats['center_pts'].append( (center[1], center[0]) ) # append tuple
            ...
            cv2.ellipse(overlay,((center[1], center[0]), ellipse[1], ellipse[2]),(255,0,0),4) #cv2.ellipse(img, (centerX,centerY), (width,height), 0, 0, 180, color, lt)
        ...
        if cv2.waitKey(int(args.delay_s*1000)) & 0xFF == ord('q'):
            break
